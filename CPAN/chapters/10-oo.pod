...
X<OO Programming> Perl always suffered from the lack of an integrated object
...
A few years ago C<Moose> appeared. It has revolutionized the OO world of
Perl. During these years it has been enriched with a lot of relevant
and useful features, many of which are hard to find elsewhere.
...
used to define method signatures as you would in other modern languages
such as Python or Ruby. Additionally, variable types can be specified as 
you might in a a typed language like C or Java (except that the type 
annotations are only optional). At the moment it is still in alpha stage, but there are
high hopes for it.
